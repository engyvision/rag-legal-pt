...
class ScraperService:
    ...
    async def scrape_historical(self, start_date: str, end_date: str):
        ...
        logger.info(
            f"Scraping historical documents from {start_date} to {end_date}")
    ...
    async def _process_document(self, document: dict):
        ...
        try:
            ...
            filename = f"{timestamp}_{doc_type}_{document.get('document_number', 'doc')}.json"
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"{retrieval_url}/api/v1/process",
                    json=payload
                ) as response:
                    if response.status == 200:
                        result = await response.json()
                        logger.info(
                            f"Document processed: {result.get('document_id')}")
                    else:
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "daily":
            ...
            logger.error(
                "Invalid command. Use: daily or historical <start_date> <end_date>")
            sys.exit(1)
    else:
